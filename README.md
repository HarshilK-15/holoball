...
|---|---|
| Clap twice within about a second and a half | The ball appears |
...
| Clap once while the ball is up | The ball gets crushed and disappears |

Two claps to summon rather than one, because a single clap is far too easy to trigger by accident.
...
Open the address it prints, then allow camera access when the browser asks. Desktop Chrome is what this has been built against.
...
Three layers stacked on top of each other. A video element showing your camera, a transparent WebGL canvas with the ball on it, and an HTML overlay for the readouts.
...
The ball itself is Three.js. An icosahedron gives it the triangulated wireframe look, glowing points sit at every vertex, three rings orbit it on different axes, and about 1200 particles drift around the outside. Bloom post-processing does the glow.
